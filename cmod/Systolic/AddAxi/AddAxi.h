...
#ifndef __Add_AXI_H__
#define __Add_AXI_H__
...
SC_MODULE(AddAxi)
{
  ...
  //
  // (N byte >> 3)*(N+1) row, 
  //   N rows of Add and 1 row of bias
  // N = 32 => 132 => 131
  // N = 16 => 34  => 33
  // N = 8  => 9   => 8
 ...
 public:
  ...
  Connections::Out<spec::InputType> add_in_vec[N]; // output add_in_vec or systolic array
  Connections::Out<bool> rd_IRQ;                      // rd_IRQ -> IRQ[4]
  
  typename spec::AxiData::axi4_data::AddrPayload addr_pld;
  typename spec::AxiData::axi4_data::ReadPayload data_pld;

  SC_HAS_PROCESS(AddAxi);
  AddAxi(sc_module_name name_) : sc_module(name_),
  ...
  void AddAxiRun() {
    ...
    while(1) {
      ...
      cout << "base_addr: " << hex << base_addr << endl;
      ...
      // Needs to ensure that we can recieve Add every cycle 
